...
class EV_Elementor_Vacations_Autofill {
    ...
    private function positions_map() {
        $opt    = get_option(EV_Elementor_Vacations::OPT, array());
        $labels = isset($opt['positions']) ? $opt['positions'] : array();

        if (is_string($labels)) {
            $labels = preg_split('/\r\n|\r|\n/', $labels);
        }

        $labels = array_values(array_filter(array_map(function ($r) {
    ...
    private function branches_map() {
        // Same logic as EV_Elementor_Vacations::branches_map (with branch_codes)
        $opt    = get_option(EV_Elementor_Vacations::OPT, array());

        // labels: from settings
        $labels = isset($opt['branches']) ? $opt['branches'] : array();
        if (is_string($labels)) {
            $labels = preg_split('/\r\n|\r|\n/', $labels);
        }
        $labels = array_values(array_filter(array_map(function ($r) {
        ...
        // custom codes: 1001, 2001, etc.
        $codes = (isset($opt['branch_codes']) && is_array($opt['branch_codes'])) ? $opt['branch_codes'] : array();
        ...
        foreach ($labels as $i => $label) {
            ...
            if ($raw_code !== '') {
                // keep only digits, so "1001 – Saburtalo" -> "1001"
                $code = preg_replace('/\D+/', '', $raw_code);
            }
            if ($code === '') {
                // fallback to simple 101 + index if no custom code set
    ...
    private function code_to_label($code, $map) {
    ...
    private function normalize_position_label($code, $posMap) {
        $code  = trim((string) $code);
        if ($code === '') return '';
        // Code comes from META_POSITION (normalized in main plugin), so just map
        return $this->code_to_label($code, $posMap);
    }

    private function normalize_branch_label($code, $brMap) {
        $code  = trim((string) $code);
        if ($code === '') return '';
        // IMPORTANT: we DO NOT fall back to raw code (1001/2001); only label
        return $this->code_to_label($code, $brMap);
    }
    ...
    private function get_employees() {
        ...
        $posMap = $this->positions_map();
        $brMap  = $this->branches_map();
        ...
        foreach ($users as $u) {
            ...
            $positionLabel = $this->normalize_position_label($posCode, $posMap);
            $branchLabel   = $this->normalize_branch_label($brCode,  $brMap);
