...
// REST
    ...
    private function list_from_setting($key): array {
        $opt  = get_option(self::OPT, []);
        $list = $opt[$key] ?? [];
        if (is_string($list)) $list = preg_split('/\r\n|\r|\n/', $list);
        $out = [];
        foreach ((array)$list as $raw) {
            $label = trim(wp_strip_all_tags((string)$raw));
            if ($label !== '' && !in_array($label, $out, true)) $out[] = $label;
        }
        return array_values($out);
    }

    private function positions_map(): array {
        $labels = $this->list_from_setting('positions');
        $map = [];
    ...
    private function code_for_position_label(string $label): ?string {
        ...
        foreach ($this->positions_map() as $code => $lab) {
            if ($lab === $label) return $code;
        }
        return null;
    }

    private function code_for_branch_label(string $label): ?string {
        $label = trim($label);
        if ($label === '') return null;
        foreach ($this->branches_map() as $code => $lab) {
            if ($lab === $label) return $code;
        }
        return null;
    }

    private function normalize_position_meta($val): string {
        $val = trim((string)$val);
        if ($val === '') return '';
    ...
    private function normalize_branch_meta($val): string {
        $val = trim((string)$val);
        if ($val === '') return '';
    ...
    private function get_user_annual_allocation(int $user_id): int {
        ...
        $default  = isset($settings['default_annual']) ? (int)$settings['default_annual'] : 24;
        $custom   = get_user_meta($user_id, self::META_ANNUAL, true);
        $annual   = $custom !== '' ? (int)$custom : $default;
        return max(0, $annual);
    }
    ...
    private function parse_ui_birthday(string $s): string {
        $s = trim($s);
        if ($s === '') return '';
        $fmts = ['d/m/Y','d-m-Y','d.m.Y','j/n/Y','j-m-Y','j.n.Y','d/m/y','j/n/y','Y-m-d'];
        foreach ($fmts as $fmt) {
            $dt = \DateTime::createFromFormat($fmt, $s);
            if ($dt && (!($e = $dt->getLastErrors()) || ($e['warning_count'] === 0 && $e['error_count'] === 0))) {
    ...
    private function format_ui_birthday(string $stored): string {
        $stored = trim($stored);
        if ($stored === '') return '';
        $norm = $this->normalize_birthday_string($stored);
        if ($norm === '') return '';
        $ts = strtotime($norm);
        return $ts ? date_i18n('d/m/Y', $ts) : '';
    }

    private function normalize_birthday_string(string $raw): string {
        $s = trim($raw);
        if ($s === '') return '';
        if (preg_match('/^\d{4}-\d{2}-\d{2}$/', $s)) return $s;
        $candidates = [
            'd/m/Y','d-m-Y','d.m.Y','j/n/Y','j-m-Y','j.n.Y',
            'Y-m-d','Y/n/j','Y-n-j','Y/m/d','Y-m-j',
        ];
    ...
    private function md_from_birthday(string $raw): string {
        $norm = $this->normalize_birthday_string($raw);
        ...
        register_rest_route('company-suite/v1', '/maps', [
            ...
            'callback' => function() {
                return new WP_REST_Response([
                    'positions' => $this->positions_map(),
                    'branches'  => $this->branches_map(),
                ], 200);
            }
        ]);
    }
    ...
    public function profile_fields($user) {
        ...
        $birthday_ui = $this->format_ui_birthday((string)$birthday);
        ?>
    ...
    public function shortcode_user_admin($atts = []) {
        ...
        $required_cap = apply_filters('company_suite/frontend_admin_cap', $a['cap']);

        // Only require login + basic cap ('read' = any logged-in user)
        if (!is_user_logged_in() || !current_user_can($required_cap)) {
            return '<div class="evua-wrap"><em>' .
                esc_html__('Please log in to view this list.', 'ev-vac') .
                '</em></div>';
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['evua_action']) && $_POST['evua_action'] === 'delete' && isset($_POST['_evua_del'])) {
                ...
                if (!is_user_logged_in()) {
                    echo '<div class="notice notice-error" style="padding:8px;margin:10px 0;">' . esc_html__('You must be logged in.', 'ev-vac') . '</div>';
                } elseif ($uid_del <= 0 || !$nonce_ok) {
                    echo '<div class="notice notice-error" style="padding:8px;margin:10px 0;">' . esc_html__('Delete failed or invalid request.', 'ev-vac') . '</div>';
                } else {
                    $this->ensure_user_admin_functions_loaded();

                    if ($this->can_delete_user_row($uid_del)) {
                        set_transient('ev_del_guard_'.$uid_del, 1, 60);
                        ...
                        $exists = get_userdata($uid_del);
                        if (!$exists) {
                            echo '<div class="notice notice-success" style="padding:8px;margin:10px 0;">' . esc_html__('User deleted.', 'ev-vac') . '</div>';
                        } else {
                            echo '<div class="notice notice-error" style="padding:8px;margin:10px 0;">' . esc_html__('Delete failed.', 'ev-vac') . '</div>';
                        }
                    } else {
                        echo '<div class="notice notice-error" style="padding:8px;margin:10px 0;">' . esc_html__('Not allowed to delete this user.', 'ev-vac') . '</div>';
                    }
                }
            }
            ...
            if (isset($_POST['_evua_nonce']) && wp_verify_nonce($_POST['_evua_nonce'], 'evua_update') && (!isset($_POST['evua_action']) || $_POST['evua_action'] === 'save')) {
                ...
                if ($edit_user > 0 && current_user_can('edit_user', $edit_user)) {
                    ...
                    if ($new_birthday_raw === '') {
                        ...
                        $norm = $this->parse_ui_birthday($new_birthday_raw);
              ...
              <tbody>
                <?php foreach ($users as $u): ?>
                  <?php
                    ...
                    $bday_ui   = $this->format_ui_birthday($bday_raw);
