...
class EV_Company_Suite_Own_Branch {
        ...
        /**
         * Get branches map code => label, same logic as main plugin
         */
        private function get_branches_map(): array {
            $opts   = get_option(EV_Elementor_Vacations::OPT, []);
            $labels = (!empty($opts['branches']) && is_array($opts['branches'])) ? $opts['branches'] : [];
            $codes  = (!empty($opts['branch_codes']) && is_array($opts['branch_codes'])) ? $opts['branch_codes'] : [];
            ...
            foreach ($labels as $i => $label) {
                ...
                // Only branches whose code starts with "1"
                if (strpos($code, '1') !== 0) {
                    continue;
                }
            ...
            // Reuse existing filter name so custom filters also apply here
        ...
        public function render_shortcode($atts = [], $content = '') {
            ...
            $branches = $this->get_branches_map();
            if (empty($branches)) {
